...
segment logic, decimated H5, norm_stats from project). Intended to be run via:
    ...
    torchrun --nproc_per_node=4 train_tcn_ddp_original.py [OPTIONS]
"""
...
def parse_args():
    ...
    # ── data (original shot-list dataset: disrupt-only, decimated) ─────
    ...
    g.add_argument('--data-step', type=int, default=10,
                   help='Decimation step (raw 1 MHz -> decimated; 10 => 100 kHz)')
    g.add_argument('--nsub', type=int, default=781_250,
                   help='Window length in raw samples (converted to decimated in dataset)')
    ...
    g.add_argument('--checkpoint-dir', type=str, default='checkpoints_tcn_ddp_original',
