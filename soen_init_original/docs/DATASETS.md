...
| Paradigm | Mapping | Task Type | Input Shape | Target Shape |
|----------|---------|-----------|-------------|--------------|
| Supervised | seq2static | Classification | `[N, T, D]` | `[N]` |
| Supervised | seq2static | Regression | `[N, T, D]` | `[N, K]` |
| Supervised | seq2seq | Classification | `[N, T, D]` | `[N, T]` |
| Supervised | seq2seq | Regression | `[N, T, D]` | `[N, T, K]` |
| Self-supervised | seq2seq | Reconstruction | `[N, T, D]` |
...
data:
  data_path: sequence_labeling.h5
  num_classes: 5
  target_seq_len: null  # Use original sequence lengths

# Note: No time_pooling needed for seq2seq tasks!
# The model uses full sequence outputs directly
```

### 3.5 Supervised seq2seq Classification (Token-Level Translation)

**Use case**: Classify each timestep independently (not autoregressive). Examples: morse code translation, speech recognition (frame-level), OCR.

**Key distinction**: This is aligned translation where every timestep has a label. Different from autoregressive next-token prediction.

**Example: Morse Code Translation**
...
# Using the morse code dataset generator
from generate_morse_dataset import generate_morse_translation_dataset

# Generate dataset with random text subsequences
generate_morse_translation_dataset(
    output_path="morse_translation.h5",
    sequence_length=15,  # Characters per sample
    num_samples=1000,
    steps_per_symbol=3,
...
# Inspect the dataset
with h5py.File("morse_translation.h5", 'r') as f:
    train_data = f['train/data'][:]  # Shape: [N, T, 2]
    train_labels = f['train/labels'][:]  # Shape: [N, T]
    
    print(f"Data shape: {train_data.shape}")
    print(f"Labels shape: {train_labels.shape}")
    # Labels are 2D! Each timestep has a character class label
```

**Label Alignment Strategy**:

For aligned translation tasks, each character label is repeated for all timesteps of its duration:

```python
# Example: Text "HI" → Morse code
# H = "...." → 21 timesteps (with silence between symbols)
# I = ".." → 9 timesteps (with silence between symbols)
# Labels: [H_class]*21 + [I_class]*9 = shape [30]

# Padding positions use -100 (ignored in cross_entropy loss)
```

**Shape requirements**:
- Data: `[N, T, D]` (input sequences)
- Labels: `[N, T]` (integer class per timestep)
- Padding: Use `-100` for padding positions (automatically ignored by cross_entropy)
...
data:
  data_path: morse_translation.h5
  num_classes: 27  # A-Z + space (use 37 if numbers included)
  target_seq_len: null  # Use original sequence lengths

# Note: No time_pooling needed - seq2seq uses full sequence outputs
model:
  # time_pooling is ignored for seq2seq tasks
```

**Training**: The `cross_entropy` loss automatically handles 2D targets by flattening:
- `outputs: [B, T, num_classes]` → `[B*T, num_classes]`
- `targets: [B, T]` → `[B*T]`
- Computes cross-entropy at every timestep independently
- Padding positions (`-100`) are automatically ignored
